...
from bert_finetune_re.utils import init_logger, load_tokenizer, get_re_labels, MODEL_CLASSES
...
def load_model(pred_config, args, device):
    ...
    try:
        model = MODEL_CLASSES[args.model_type][1].from_pretrained(args.model_dir,
                                                                  args=args,
                                                                  re_label2id=get_re_labels(args),
                                                                  )
        model.to(device)
        model.eval()
...
def predict(pred_config):
    ...
    for batch in tqdm(data_loader, desc="Predicting"):
        ...
        with torch.no_grad():
            inputs = {"input_ids": batch[0],
                      ...
                      "re_label_ids": None,
                      'head_entity_pos': batch[4],
                      'tail_entity_pos': batch[5],
                      }
    ...
    with open(pred_config.output_file, "w", encoding="utf-8") as f:
        for words, re_pred in zip(lines, re_preds):
            line = ""
            f.write("{}\n".format(re_id2label[re_pred]))
