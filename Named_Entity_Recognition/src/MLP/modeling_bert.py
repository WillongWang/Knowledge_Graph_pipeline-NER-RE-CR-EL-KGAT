...
from src.Biaffine.loss_function_utils import FocalLoss
...
class BertForTokenClassificationWithPabeeMLP(BertPreTrainedModel):
            ...
            for ix, logits_item in enumerate(logits):
                if self.num_labels == 1:
                    ...
                    if attention_mask is not None:
                        ...
                        active_logits = logits_item.view(-1, self.num_labels)[
                            active_loss]  # (有效长度， num_slot_labels)
                        active_labels = labels.view(-1)[active_loss]  # (有效长度， )
                        loss = loss_fct(active_logits, active_labels)
                    else:
                        loss = loss_fct(
                            logits_item.view(-1, self.num_labels),
                            labels.view(-1)
                        )
                ...
                total_weights += ix + 1
            outputs = (total_loss / total_weights,) + (outputs[0],)
