...
from src.MLP.modeling_bert import BertForTokenClassificationWithPabeeMLP
from src.MLP.finetuning_argparse import get_argparse
...
MODEL_CLASSES = {
    ...
    "bertmlp": (BertConfig, BertForTokenClassificationWithPabeeMLP, BertTokenizer),
...
def train(args, train_dataset, model, tokenizer, labels):
    ...
    classifiers_param_optimizer = list(model.classifiers.named_parameters())
    optimizer_grouped_parameters += [
        {
            'params': [p for n, p in classifiers_param_optimizer if not any(nd in n for nd in no_decay)],
            'weight_decay': args.weight_decay,
            'lr': args.classifiers_learning_rate
        },
        {
...
def evaluate(args, model, tokenizer,
    ...
    tokens_list = [[] for _ in range(out_slot_labels_ids.shape[0])]
    ...
    if args.eval_all_checkpoints and patience != 0:
        ...
        elif args.model_type == "bertmlp":  ### args.model_type == "bert":
...
def load_and_cache_examples(args, tokenizer, labels, mode):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        print("args.sub_token_label_scheme: ", args.sub_token_label_scheme)
        features = convert_MRC_examples_to_featuresCRF(examples, labels, args.max_seq_length, tokenizer,
                                                       ...
                                                       sub_token_label_scheme=args.sub_token_label_scheme,
                                                       )
        if args.local_rank in [-1, 0]:
