...
class ProductIndexMap(IndexMap):
    ...
    def __init__(self, outer_index, inner_index):
        """
        Combines indices i and j into pairs (i, j). The results is an index where each segment (i, j) is the
...
def flatten(index, name="segmented_flatten"):
    ...
    `num_segments` * (k - 1). The results is a tensor with `num_segments` multiplied by the number of elements in the
    batch.
...
def _calculate_expected_result(
    ...
    """
    Calculates the expected results given cell and aggregation probabilities.
    ...
    Returns:
        expected_result (:obj:`torch.FloatTensor` of shape :obj:`(batch_size,)`): The expected results per example.
    """
